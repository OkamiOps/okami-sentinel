...
Portable `deep` enumerates the auditable source and security-configuration universe from the immutable snapshot and supplies complete files in server-owned discovery partitions. Every partition must finish; this records supplied source coverage, not proof that a model understood every line. It never assumes a prior `standard` result. Portable `standard` uses bounded, directed exploration and must report its inspected and unexamined scope honestly; it does not promise Deep coverage.
...
|---|---:|---:|
| `minimal`, `low` | 24 turns / 96 tool calls | 48 turns / 384 tool calls |
| `medium`, `high`, unknown, or absent | 32 turns / 128 tool calls | 64 turns / 512 tool calls |
| `xhigh` | 48 turns / 192 tool calls | 96 turns / 768 tool calls |
| `max`, `ultra` | 64 turns / 256 tool calls | 128 turns / 1,024 tool calls |

There is no elapsed-time expiry in either mode. Cancellation remains available and an optional cost ceiling is scan-global. Turn and tool values bound a base session; discovery, assessment and report pages have bounded per-page policies without resetting the cost budget. Base Portable sessions are capped at 64 MiB of input and 1 MiB of output; exhaustive Deep coverage partitions may use a dedicated 16 MiB output guard, while the canonical public report remains capped at 4 MiB. An optional `maxCostUsd` ceiling requires a frozen matching price quote at launch; without one, launch fails closed. Reported usage that reaches the ceiling stops the session before the next provider request, although one request already in flight can exceed the estimate; usage that cannot be estimated stops rather than claiming the ceiling was enforced.
