...
Portable não afirma que um provider que não é OpenAI está executando o scanner upstream. Ele executa `sentinel/codex-security-methodology@v1`: seis estágios defensivos e somente estáticos sobre um snapshot imutável, com ferramentas limitadas de leitura/busca, artefatos estruturados, cancelamento e isolamento obrigatórios. Um dossiê estruturado, mantido pelo servidor, carrega inventário, conjunto de candidatos, resumos das etapas, avaliações e escopo entre os estágios. Páginas internas do relatório são limitadas a 4 candidatos confirmados cada, validadas antes de I/O e podem usar apenas reparo limitado dentro dos limites do scan; o servidor publica somente o relatório final consolidado. Findings retidos precisam partir de um candidato já carregado e conter `rootCause`, impacto, remediação e anchors sustentados pelo repositório; um relatório com zero findings só é válido quando a cobertura registra explicitamente cada candidato e o escopo inspecionado ou não inspecionado. Antes do lançamento, a tupla persistida exata de conexão/modelo/protocolo precisa ter uma capability probe nova e aprovada. Probe ausente, expirada, falha ou incompatível bloqueia o run; o Sentinel nunca faz fallback para Native, outra rota, CLI ou outro modelo.

Novas escritas de descoberta Portable exigem `candidates` explícito e contexto limitado da alegação: hipótese, atacante, pré-requisitos, impacto esperado e falha de controle suspeita. A validação pode consultar chamadores e mitigações em outros arquivos. Hipóteses sem sustentação permanecem como `insufficient-evidence`, fora dos achados confirmados; não viram automaticamente vulnerabilidades medium/low. Novos achados high/critical exigem justificativa de severidade. São verificações do contrato de evidência, não garantia de precisão do modelo.

Runs Deep densos mantêm assessment e report limitados sem truncar o resultado final. Dataflow processa candidatos em páginas controladas pelo servidor com no máximo 32 candidatos; validation usa páginas de até oito. O report usa páginas privadas, somente de findings, com no máximo 4 candidatos confirmados; a cobertura dos candidatos rejeitados e o escopo inspecionado continuam derivados pelo servidor. O planejador cria as páginas necessárias aos candidatos confirmados; o normalizador canônico aceita até 512 findings. Cada página é validada contra sua associação exata ao dossiê e contra os anchors fixados do repositório antes de I/O; somente o `sentinel-findings.json` consolidado e o `findings.json` público normalizado são publicados.

### Graphify, cobertura e recuperação

Novos scans completos Standard e Deep enumeram o mesmo universo auditável de código/configuração e projetam os arquivos atribuídos completos. O Graphify agrupa arquivos relacionados e fornece relações do código; arquivos sem nós no grafo continuam incluídos. Standard revisa os caminhos principais com effort `low` por padrão; Deep investiga caminhos alternativos e contraevidências com `high`. Escolhas explícitas suportadas são preservadas; rotas sem controle de effort usam o default do provider. Código fornecido não comprova atenção do modelo nem qualidade de detecção. Cada scan completo é independente; a recuperação reutiliza apenas checkpoints validados daquele run.

O Sentinel gerencia o Graphify: pnpm provisiona seu runtime isolado e Docker o inclui na imagem. A projeção de código para avaliação e relatório se adapta ao contexto disponível após reservar instruções, resposta e continuação. Não há quantidade fixa de trechos nem corte por tamanho de função; intervalos pendentes ficam disponíveis para leituras direcionadas. O guard de contexto estimado usa o menor valor entre 300K tokens e a janela do modelo. Relações do grafo orientam a navegação, nunca provam vulnerabilidades.

Standard e Deep não têm teto acumulado de chamadas de ferramenta ou turnos do modelo. Inspeções repetidas recebem orientação contra loops; exploração produtiva pode continuar. Validação de evidências, controles de bytes/contexto, cancelamento e teto opcional de custo global permanecem. Falhas recuperáveis usam unidades menores com histórico novo e checkpoints validados, respeitando orçamentos persistentes de tentativas; erros terminais e cancelamentos explícitos não são repetidos cegamente. O preview ao vivo mostra candidatos provisórios sem aumentar a contagem de achados confirmados.

Veja a [arquitetura e os resultados medidos](docs/architecture/portable-graph-recovery.md). O Deep adaptativo mais recente concluiu em cerca de 5h21 com 36 achados reportados, contra 7h07 e 31 no mesmo snapshot com a integração Graphify anterior. É um ganho observado, não um experimento isolando Graphify nem garantia de precisão. O Standard ainda não foi medido com a projeção adaptativa mais recente. As metas de benchmark de 90 minutos para Standard e cinco horas para Deep não são timeouts de execução.
...
flowchart LR
    ...
    VAULT[("Cofre do SO / vault criptografado do servidor")]
...
| Opção | Uso recomendado | Rotas de execução suportadas |
|---|---|---|
| [pnpm](#instalação-via-pnpm) | Estação local e desenvolvimento; requer Node 24 e pnpm 11.5.2 | Sessões CLI locais e rotas HTTP com capability aprovada |
| [Docker Compose](#instalação-via-docker) | Servidor próprio Linux amd64; dispensa Node e pnpm no host | Codex Security Portable, Mantis HTTP e VulnHunter HTTP |

Cada instalação Docker possui seus próprios dados e credenciais; não é um SaaS multi-tenant. Native e sessões CLI locais continuam disponíveis via pnpm. Consulte as mudanças recentes no [changelog](CHANGELOG.md).
...
Mutações na API local exigem o token da sessão do processo, obtido em `GET /api/security-session`, no header `X-CSRF-Token`, inclusive em clientes CLI. Origens de navegador ficam limitadas à aplicação local e ao frontend Vite suportado; a interface envia o token automaticamente.

## Instalação via Docker

Requer Docker Engine com Docker Compose v2 em Linux amd64. Interface, API e workers rodam no mesmo container, com SQLite e estado privado em volume persistente. O setup usa um container temporário de Node 24; não exige Node ou pnpm no host.
...
Abra <http://127.0.0.1:8787> e entre com o usuário `admin`. A senha fica no arquivo `~/.local/share/okami-sentinel/admin_password`, fora do checkout; o setup não imprime secrets. Configure as conexões de provider no vault do servidor.

Por padrão, o setup autoriza o checkout do Sentinel. Para analisar outro projeto, use `sh scripts/docker/setup.sh --repository /caminho/do/projeto` no primeiro setup e selecione `/repos/projeto` na interface. O caminho de origem precisa existir no host Docker; abrir o navegador não envia arquivos do visitante ao servidor.

O acesso local fica restrito a `127.0.0.1:8787`. Para hospedagem com domínio HTTPS, siga o [guia do Dokploy](docs/dokploy.md). Consulte o [guia Docker](docs/docker.md) para portas alternativas, backup, restauração e atualização. O suporte validado é Linux amd64 com as três engines HTTP; Native, sessões CLI do host, Apple Silicon, Windows e Linux arm64 não fazem parte da validação inicial.
