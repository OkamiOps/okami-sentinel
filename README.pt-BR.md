...
O `deep` Portable enumera no snapshot imutável o universo auditável de código e configurações de segurança e fornece arquivos completos em partições de descoberta controladas pelo servidor. Todas as partições precisam concluir; isso registra a cobertura de código fornecido, não prova que o modelo compreendeu cada linha. Não pressupõe um resultado `standard` anterior. O `standard` Portable usa exploração direcionada e limitada e deve registrar honestamente o escopo inspecionado e não inspecionado; não promete a cobertura do Deep.
...
|---|---:|---:|
| `minimal`, `low` | 24 turnos / 96 chamadas de ferramenta | 48 turnos / 384 chamadas de ferramenta |
| `medium`, `high`, desconhecido ou ausente | 32 turnos / 128 chamadas de ferramenta | 64 turnos / 512 chamadas de ferramenta |
| `xhigh` | 48 turnos / 192 chamadas de ferramenta | 96 turnos / 768 chamadas de ferramenta |
| `max`, `ultra` | 64 turnos / 256 chamadas de ferramenta | 128 turnos / 1.024 chamadas de ferramenta |

Nenhum dos modos expira por tempo decorrido. O cancelamento continua disponível e o teto opcional de custo é global ao scan. Turnos e ferramentas limitam uma sessão base; páginas de descoberta, avaliação e relatório têm limites próprios sem renovar o orçamento de custo. Sessões Portable base são limitadas a 64 MiB de entrada e 1 MiB de saída; partições de cobertura Deep exaustiva podem usar um guard dedicado de 16 MiB de saída, enquanto o relatório público canônico permanece limitado a 4 MiB. Um teto opcional `maxCostUsd` exige cotação correspondente congelada no lançamento; sem ela, o lançamento falha de forma fechada. Usage reportado que alcança o teto encerra a sessão antes da próxima request ao provider, embora uma request já em voo possa exceder a estimativa; usage que não pode ser estimado encerra em vez de alegar que o teto foi aplicado.
