...
Portable `deep` est exhaustif de manière autonome : il ne consomme ni ne suppose jamais un résultat `standard` antérieur. Sentinel énumère dans le snapshot immuable tout l’univers auditable du code source et des configurations de sécurité, le partitionne côté serveur et exige un `workspace.read` complet et réussi pour chaque fichier attribué avant de terminer la découverte. Un fichier ou une partition manquante provoque un échec fermé au lieu de publier un rapport Deep partiel. `standard` reste le mode d’exploration borné et moins coûteux ; il ne promet donc pas la même couverture exhaustive.
