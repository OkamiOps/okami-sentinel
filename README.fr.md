...
![Séquenceur de lancement OKAMI Sentinel avec Codex Security, Google Mantis, Capital One VulnHunter, modèle fournisseur en direct et autorisation preflight](docs/assets/okami-sentinel-overview.png)
...
- **Contrôles de runtime dynamiques** — modèle, niveaux d’effort de raisonnement pris en charge, mode de scan et frontière d’exécution sont résolus par les capacités de la connexion et du moteur sélectionnés ; les valeurs non prises en charge ne sont pas codées en dur dans le formulaire de lancement. Le profil d’exécution indique si un effort a été envoyé, quel champ wire a été utilisé ou si la valeur par défaut du fournisseur a été conservée.
...
- **Inspection centrée sur les preuves** — filtres de sévérité et de lifecycle, résumés, emplacements et chemins d’attaque, avec un marquage explicite lorsqu’un finding ne contient pas de preuve structurée jointe.
- **Registre de runs lisible** — chaque run affiche des badges de moteur et de modèle ainsi que `High+` et le total de vulnérabilités ; seuls les runs terminaux peuvent être supprimés du registre.
...
| Moteur | État | Routes de connexion exécutables | Modèles | Frontière d’exécution |
|---|---|---|---|---|
| [`@openai/codex-security`](https://github.com/openai/codex-security) | Stable | Session locale OpenAI Codex/ChatGPT ou API OpenAI | Catalogue authentifié en direct | Scan standard ou deep ; plafond USD explicite pris en charge |
| [Google Mantis](https://github.com/google/mantis) | Preview | Session Codex/ChatGPT, session locale Claude Code, OAuth xAI direct et fournisseurs HTTP prouvés par capacité | Catalogue authentifié en direct ; Claude peut utiliser son défaut de runtime explicite | Neuf étapes scan-only déterministes sur un snapshot immuable |
| [Capital One VulnHunter](https://github.com/capitalone/vulnhunter) | Expérimental | Session Codex/ChatGPT, OAuth xAI direct et fournisseurs HTTP prouvés par capacité | Catalogue authentifié en direct | Profil de compatibilité statique, en lecture seule, à six étapes issu de la méthodologie VulnHunter revue |

Mantis est récupéré à un commit revu, validé puis publié atomiquement dans un cache local privé. La première phase exclut délibérément `mantis-reproduce`, `mantis-chain` et `mantis-patch` : l’adapter n’écrit pas dans le dépôt cible et n’exécute pas de code d’exploit généré. Les routes HTTP passent par l’hôte d’outils agent borné de Sentinel. Les runs d’abonnement Claude Code utilisent un répertoire de session vide et séparé, sans outils intégrés, ainsi qu’un seul serveur MCP privé en lecture seule qui expose uniquement des opérations bornées de liste, lecture et recherche sur le snapshot immuable. L’état brut de Mantis reste à côté des preuves Sentinel normalisées pour l’audit.

Le flux upstream de VulnHunter est orienté Claude et inclut des étapes de vérification opérationnelles qui peuvent déclencher les protections cyber d’un fournisseur. Sentinel enregistre donc la révision upstream revue comme provenance de son profil local versionné séparément, mais **ne** récupère ni n’envoie le skill upstream ou ses prompts de phase à Codex au runtime. Son profil de compatibilité expérimental conserve la forme utile — reconnaissance, traces statiques vers l’avant, falsification adversariale, balayage de couverture et remédiation étayée par des preuves — dans une seule session en lecture seule sur un snapshot immuable. Les findings retenus sont normalisés vers le même contrat de preuve Inspector que les autres moteurs. Une politique de fournisseur peut encore refuser une revue de dépôt ; Sentinel conserve alors le journal complet du run, retient l’usage de tokens déjà rapporté par l’app-server Codex et signale l’exigence Trusted Access sans prétendre que le scan est terminé. Si le fournisseur s’arrête avant de rapporter l’usage, le coût reste indisponible au lieu d’apparaître comme un faux run à zéro dollar.

> [!NOTE]
> Abonnement, OAuth, Token Plan et facturation API sont des routes distinctes. Sentinel lie chaque scan à une connexion persistée et soit à un modèle découvert en direct, soit à un runtime par défaut déclaré par l’adapter, revalide cette sélection avant d’accéder aux credentials et ne bascule jamais silencieusement vers une autre route.
...
Portable ne prétend pas qu’un fournisseur autre qu’OpenAI exécute le scanner upstream. Il exécute `sentinel/codex-security-methodology@v1` : six étapes défensives, uniquement statiques, sur un snapshot immuable, avec outils de lecture/recherche limités, artefacts structurés, annulation et isolation obligatoires. Un dossier structuré géré par le serveur transmet l’inventaire, l’ensemble des candidats, les résumés d’étapes, les évaluations et le périmètre entre les étapes. Avant le lancement, le tuple persistant exact connexion/modèle/protocole doit avoir une capability probe fraîche et réussie. Une probe absente, périmée, échouée ou non correspondante bloque le run ; Sentinel ne bascule jamais silencieusement vers Native, une autre route, une CLI ou un autre modèle.

Les nouveaux artefacts de découverte Portable exigent un tableau `candidates` explicite et un contexte borné : hypothèse, attaquant, prérequis, impact attendu et contrôle supposé défaillant. La validation peut consulter les appelants et protections dans d’autres fichiers. Les hypothèses non étayées restent des dispositions `insufficient-evidence`, hors des findings confirmés ; elles ne deviennent pas automatiquement des vulnérabilités medium/low. Les nouveaux findings high/critical exigent une justification de sévérité. Ces contrôles du contrat de preuve ne garantissent pas la précision du modèle.

La phase de rapport segmente en interne les candidats confirmés par pages de 4 candidats au maximum. Une page modèle ne peut livrer que ses findings ; la couverture et les candidats rejetés restent gérés par le serveur. Avant tout `results.write`, Sentinel valide le JSON, le contrat de l’étape, la sémantique du dossier, l’appartenance à la page, la couverture ainsi que les chemins et plages de lignes des ancres du snapshot. Les artefacts rejetés reçoivent des indications de correction précises ; les écritures invalides identiques répétées restent soumises au contrôle de stagnation. Seul le rapport final assemblé par le serveur est écrit comme `sentinel-findings.json` ; les artefacts de page ne sont pas publiés comme résultat Sentinel.

Dataflow traite les candidats par pages de 32 au maximum ; Validation utilise des pages de huit au maximum. Le planificateur crée les pages privées nécessaires aux candidats confirmés ; le normaliseur canonique accepte jusqu’à 512 findings et publie un seul `findings.json` normalisé. L’appartenance à la page et les ancres du dépôt sont vérifiées avant I/O, afin qu’un run Deep dense ne tronque jamais silencieusement des candidats et ne publie aucune page partielle.
...
### Graphify, couverture et reprise

Les nouveaux scans complets Standard et Deep recensent le même ensemble auditable de fichiers sources et de configuration et fournissent intégralement les fichiers affectés. Graphify regroupe les fichiers liés ; les fichiers absents du graphe restent inclus. Standard examine les chemins principaux avec un effort `low` par défaut ; Deep explore les chemins alternatifs et les contre-preuves avec `high`. Les choix explicites pris en charge sont conservés ; sans réglage d’effort, le fournisseur applique son comportement par défaut. Le code fourni ne prouve ni l’attention du modèle ni la qualité de détection. Chaque scan complet est indépendant ; la reprise réutilise uniquement les checkpoints validés de ce run.

Sentinel gère Graphify : pnpm prépare son runtime isolé et Docker l’intègre dans l’image. Les projections de code pour l’évaluation et le rapport s’adaptent au contexte disponible, avec des réserves pour les instructions, la réponse et la continuation. Aucun nombre fixe d’extraits ni seuil de longueur de fonction ne s’applique ; les plages restantes restent accessibles par lecture ciblée. La limite de contexte estimée est le minimum entre 300K tokens et la fenêtre du modèle. Les relations du graphe facilitent la navigation sans prouver une vulnérabilité.

Standard et Deep n’ont pas de plafond cumulatif d’appels d’outils ou de tours du modèle. Les inspections répétées reçoivent des indications contre les boucles ; une exploration productive peut continuer. Les contrôles de preuve, d’octets/contexte, l’annulation et le plafond de coût global facultatif restent actifs. Les erreurs récupérables utilisent de plus petites unités, un historique neuf et des checkpoints validés dans des budgets persistants de tentatives. Les erreurs terminales et annulations explicites ne sont pas répétées aveuglément. L’aperçu en direct montre des candidats provisoires sans augmenter les findings confirmés.

Voir [l’architecture et les mesures](docs/architecture/portable-graph-recovery.md). Le dernier Deep adaptatif a terminé en environ 5h21 avec 36 findings rapportés, contre 7h07 et 31 sur le même snapshot avec l’intégration Graphify précédente. Ce gain observé ne constitue ni une expérience isolée ni une garantie de précision. Standard n’a pas encore été mesuré avec la dernière projection adaptative. Les objectifs de benchmark de 90 minutes pour Standard et cinq heures pour Deep ne sont pas des timeouts.
...
`standard` et `deep` sont des modes de scan, pas des types de scan incrémental. Le détail ne montre que les findings de l’exécution actuelle : une baseline explicite, ou automatiquement le dernier run antérieur terminé de la même lignée d’analyse (moteur, fournisseur, modèle, mode, effort, profil, route, protocole et recipe), les classe `new`, `persisting` ou `regressed`. L’absence d’un finding dans un nouveau run n’est pas une remédiation et ne s’affiche pas comme `fixed`. `fixed` reste réservé à une future comparaison incrémentale explicite.
...
flowchart LR
    ...
    MANTIS["Adapter Mantis scan-only\nskills épinglés + snapshot"]
    VULNHUNTER["Profil de compatibilité VulnHunter\nversionné localement + traces statiques"]
...
- Un coffre de credentials de système d’exploitation pris en charge par l’adapter de keychain local pour les connexions avec secrets
- Au moins une route configurée sous **Paramètres → Connexions**. Les presets disponibles incluent :
  - OpenAI Codex local, authentification ChatGPT par navigateur/appareil et API OpenAI ;
  - détection locale xAI Grok, OAuth d’appareil orchestré localement par Sentinel et API xAI ;
  - session locale Claude Code et API Anthropic ;
...
Ouvrez **Paramètres → Connexions** pour ajouter une route, l’authentifier et actualiser son catalogue de modèles. Les routes d’abonnement local utilisent toujours leur connexion officielle :
...
# Mantis et VulnHunter hébergés par Codex utilisent la session Codex générique
codex login

# Mantis local avec Claude utilise la session Claude Code existante
claude auth login
```

Au démarrage, l’API indexe les scans compatibles déjà présents dans le répertoire d’état Codex Security configuré.

Les mutations de l’API locale exigent le jeton de session du processus obtenu via `GET /api/security-session` dans l’en-tête `X-CSRF-Token`, y compris pour les clients CLI. Les origines navigateur sont limitées à l’application locale et au frontend Vite pris en charge ; l’interface transmet automatiquement le jeton.
...
## Connexions de fournisseurs

| Famille de fournisseur | Routes de connexion | Disponibilité du scanner |
|---|---|---|
| **OpenAI** | Codex local, OAuth ChatGPT dans le navigateur, code d’appareil ChatGPT, clé API | Codex Security, Mantis et VulnHunter selon la route résolue |
| **xAI** | OAuth d’appareil orchestré localement par Sentinel, clé API, détection locale de Grok | OAuth/API peut exécuter Codex Security Portable lorsque le tuple exact connexion, modèle et protocole possède une capability probe fraîche et réussie ; Mantis et VulnHunter restent dépendants de leurs capacités. Le scan local Grok reste bloqué tant que sa surface d’exécution plugin/hook ne peut pas être isolée. |
| **Anthropic** | Session Claude Code existante, API Anthropic | Claude local exécute Mantis à travers la frontière de snapshot MCP-only. L’API Anthropic peut exécuter Codex Security Portable lorsque le tuple exact possède une capability probe fraîche et réussie ; les autres moteurs restent dépendants de leurs capacités. |
...
| `VULNHUNTER_REPOSITORY_URL` | `https://github.com/capitalone/vulnhunter.git` | Dépôt source VulnHunter enregistré comme provenance de méthodologie |
...
| `CSB_HOST` | `127.0.0.1` | Adresse de boucle locale uniquement ; les adresses publiques/LAN sont refusées car l’API locale n’a pas d’authentification globale. Ne pas l’exposer via un proxy ou tunnel sans contrôle d’accès authentifié. |
...
Les tests navigateur couvrent les ordinateurs, les mobiles et les cinq langues de l’interface avec une API simulée. Ils ne lancent aucun scan externe et n’utilisent aucun identifiant réel de fournisseur.
...
> [!WARNING]
> Les scans peuvent être coûteux. Codex Security Native associe son enveloppe de coût à la protection upstream `--max-cost`. Portable peut appliquer son propre plafond USD à partir d’un devis figé après chaque événement d’usage ; il bloque la requête suivante, mais une requête déjà en vol peut encore dépasser l’estimation. Lorsque l’usage et le devis correspondant existent, Sentinel affiche une estimation ou un équivalent PAYG — pas une facturation réelle ni une facture. Les routes d’abonnement et de session locale restent **indisponibles**, jamais `$0`, lorsque le fournisseur ne rapporte pas d’usage facturable. Pour une correspondance exacte de modèle, OpenRouter estime séparément l’input non mis en cache, les lectures de cache, les écritures de cache et l’output. Les quotas de forfait, crédits et la facturation finale du fournisseur restent des mesures différentes.

- Les métadonnées et preuves normalisées restent locales. Les credentials de fournisseur sont stockés localement et utilisés uniquement pour authentifier les requests de la connexion sélectionnée. Cette route d’inférence reçoit les prompts et preuves de dépôt nécessaires au scan ; publier un GitHub Check est une action explicite distincte.
- Une erreur opérationnelle ne devient jamais une décision de sécurité positive.
- La suppression d’un scan est explicite et disponible seulement après un état terminal. Elle peut retirer l’enregistrement de l’application et le répertoire de scan associé **uniquement lorsque ce répertoire est géré par Sentinel** ; elle ne supprime jamais le dépôt cible.
