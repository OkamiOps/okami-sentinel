...
Portable `deep` inventorie les fichiers sources et de configuration de sécurité auditables dans le snapshot immuable, puis fournit les fichiers complets dans des partitions de découverte gérées par le serveur. Toutes les partitions doivent terminer. Cela atteste le code fourni, pas la compréhension de chaque ligne par le modèle. Aucun résultat `standard` préalable n’est supposé. Portable `standard` utilise une exploration ciblée et bornée et doit déclarer honnêtement les périmètres examinés et non examinés ; il ne promet pas la couverture de Deep.
