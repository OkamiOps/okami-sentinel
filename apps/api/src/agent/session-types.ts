      ...
      ? artifactRepairDetail?.kind === "json"
        ...
        ? artifactRepairDetail?.kind === "assessment-contract"
          ...
          : artifactRepairDetail?.kind === "candidate-contract"
            ? "Discovery candidates must be an array of at most 100 objects with id, category, anchors and the declared hypothesis, attacker, prerequisites, expectedImpact and controlHypothesis fields. Preserve the security claim and correct the indicated item; never remove candidates to bypass validation. IDs must be unique. Anchors contain path, startLine, endLine, role and optional explanation. For array-or-limit, use repair.field, repair.expected and repair.actualType: payload must be a JSON object and candidates must be an explicit array, including [] only when the review found no leads. A missing or wrong-type candidates field must be repaired from the existing review, not replaced with an empty placeholder. For an oversized array, repair.count and repair.limit identify the structural limit. For other errors, correct repair.itemIndex and repair.reason; context fields hypothesis, prerequisites, expectedImpact and controlHypothesis require 24–512 UTF-8 bytes after trimming, not just a short label. attacker must be unauthenticated, authenticated, privileged, local or unknown. Do not restart discovery for structural repairs."
