...
export function buildPortableCodexSecurityStagePrompt(
  ...
  return [
    ...
    ...(input.scanMode === undefined || stage.id === "report" ? [] : [
      input.scanMode === "standard"
        ? "STANDARD INVESTIGATION DEPTH: use a concise first-pass review across the entire assigned scope. Map the main input-to-control-to-sink paths using the supplied source and graph; do not enumerate every hypothetical variant. During dataflow and validation, resolve each candidate's concrete claim through the relevant caller, effective control and sink. Once source evidence establishes or refutes that claim, write the decision without exploring unrelated alternatives. A missing decisive control or caller still requires a focused source lookup; speed never justifies an unsupported confirmation or rejection."
        : "DEEP INVESTIGATION DEPTH: review the entire assigned scope and investigate materially distinct paths: alternate callers and registration, middleware ordering, authorization bypasses, data transformations, state transitions and deployment prerequisites where relevant. During dataflow and validation, actively seek counterevidence and realistic alternative paths before deciding each candidate. Use graph relationships to locate the actual source for these checks, not to assume a path is exploitable. Stop when relevant hypotheses are resolved; do not repeat equivalent paths merely to spend the budget.",
      "Graph navigation is a work-saving index: start with the supplied source and relationships, query only unresolved connections, and read missing source ranges directly. Missing graph edges do not prove that a flow is absent. Both modes require the same repository-backed standard of evidence for confirmed findings; investigation breadth of alternatives differs, not severity or truth requirements.",
    ]),
    ...(stage.id === "discovery" ? [
    ...
    stage.id === "report"
      ...
      : "The JSON must be complete in one tool call. Never exhaust the model output limit. Stage artifacts must use observations: [] and must include the structured fields required by the current stage contract: scope for inventory/threat-model/discovery, candidates for discovery, and assessments for dataflow/validation. The server forwards compact stage summaries, candidate claims, scope paths, reason codes, and line anchors; never embed source snippets or secrets in those fields.",
    stage.id === "report"
