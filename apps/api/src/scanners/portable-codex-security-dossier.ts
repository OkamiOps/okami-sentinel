...
const CANDIDATE_STAGES = new Set(["discovery", "dataflow", "validation"]);
...
interface PortableReportFinding {
  id: string;
  candidateId: string;
  title: string;
...
interface PortableReportCoverageEntry {
  candidateId: string;
...
export function normalizePortableCodexSecurityStageArtifact(
  ...
  if (artifactPath === "sentinel-findings.json") {
    ...
    return report === null ? null : report as unknown as Record<string, unknown>;
  }
  const expectedStage = stageForArtifact(artifactPath);
  if (expectedStage === null) return null;
  const artifact = parsePortableStageArtifact(value, expectedStage);
  return artifact === null ? null : artifact as unknown as Record<string, unknown>;
}
...
export function validatePortableCodexSecurityReportCoverage(
  ...
  if (dossier.scope.inspected.some((entry) => !reportInspected.has(entry)) ||
    ...
    throw new PortableCodexSecurityDossierError("coverage scope is incomplete");
  }
  ...
  for (const coverage of report.coverage.candidates) {
    if (!carriedCandidateIds.has(coverage.candidateId) || coverageByCandidate.has(coverage.candidateId)) {
      throw new PortableCodexSecurityDossierError("coverage references an unknown or duplicate candidate");
    }
  ...
  for (const finding of report.findings) {
    if (findingIds.has(finding.id) || !carriedCandidateIds.has(finding.candidateId)) {
      throw new PortableCodexSecurityDossierError("finding is not backed by a unique carried candidate");
    }
    findingIds.add(finding.id);
    if (reportedCandidates.has(finding.candidateId)) {
      throw new PortableCodexSecurityDossierError("candidate may produce only one report finding");
    }
    reportedCandidates.add(finding.candidateId);
    if (coverageByCandidate.get(finding.candidateId)?.disposition !== "reported") {
      throw new PortableCodexSecurityDossierError("reported finding is missing matching coverage");
    }
  }
  for (const [candidateId, coverage] of coverageByCandidate) {
    if (coverage.disposition === "reported" && !reportedCandidates.has(candidateId)) {
      throw new PortableCodexSecurityDossierError("reported coverage is missing its finding");
    }
    if (coverage.disposition === "rejected" && reportedCandidates.has(candidateId)) {
      throw new PortableCodexSecurityDossierError("rejected candidate cannot also be a finding");
    }
  }
  return report;
}
...
function parsePortableStageArtifact(
  ...
  if (record === null || record.schemaVersion !== 1 || !isStage(record.stage) ||
      (expectedStage !== undefined && record.stage !== expectedStage) ||
  ...
  if (summary === null || !Array.isArray(record.observations) || record.observations.length !== 0) return null;
  const scope = record.scope === undefined ? undefined : parseScope(record.scope);
  if (record.scope !== undefined && scope === null) return null;
  const candidates = record.candidates === undefined ? undefined : parseCandidates(record.candidates);
  if (record.candidates !== undefined && candidates === null) return null;
  const assessments = record.assessments === undefined ? undefined : parseAssessments(record.assessments);
  if (record.assessments !== undefined && assessments === null) return null;
  if (!CANDIDATE_STAGES.has(record.stage) && (candidates?.length ?? 0) > 0) return null;
  if (!ASSESSMENT_STAGES.has(record.stage) && (assessments?.length ?? 0) > 0) return null;
...
function parseCandidates(value: unknown): PortableCandidate[] | null {
  ...
  for (const item of value) {
    const record = asRecord(item);
    if (record === null || !hasOnlyKeys(record, new Set(["id", "category", "anchors"]))) return null;
    ...
    if (id === null || category === null || anchors === null || ids.has(id)) return null;
    ids.add(id);
...
function parseScope(value: unknown): PortableScope | null {
  const record = asRecord(value);
  if (record === null || !hasOnlyKeys(record, new Set(["inspected", "unexamined"]))) return null;
  ...
  for (const item of record.inspected) {
    const target = repositoryPath(item);
    if (target === null || inspectedSet.has(target)) return null;
  ...
  for (const item of record.unexamined) {
    ...
    if (entry === null || !hasOnlyKeys(entry, new Set(["path", "reason"]))) return null;
    const target = repositoryPath(entry.path);
    const reason = enumValue(entry.reason, COVERAGE_REASONS);
    const key = target === null || reason === null ? null : `${target}\u0000${reason}`;
    if (target === null || reason === null || key === null || unexaminedSet.has(key)) return null;
