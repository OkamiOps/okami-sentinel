...
export function RepositoryDirectoryBrowser({
  ...
  const openDirectory = useCallback(async (path?: string) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("guardrails.directoryOpenError"));
    } finally {
      setLoading(false);
    }
  }, [onChange, t]);
    ...
    <div className="overflow-hidden border" aria-busy={loading}>
      ...
      {error && (
        <div role="alert" className="flex items-center justify-between gap-3 border-b border-destructive/50 bg-destructive/[.08] px-3 py-2 text-xs text-destructive">
          <span>{error}</span>
          <Button type="button" variant="outline" className="min-h-9" onClick={() => void openDirectory(manualPath || undefined)}>{t("common.retry")}</Button>
        </div>
      )}
      ...
      <ScrollArea className="h-64">
        <div className="min-h-full">
          {loading && !directory ? (
            <div className="grid h-64 place-items-center font-mono text-[10px] text-muted-foreground">{t("guardrails.readingDirectories")}</div>
            ...
            >
              ...
              <span className="flex items-center gap-1 font-mono text-[8px] text-muted-foreground">{t("guardrails.openDirectory")} <ChevronRight aria-hidden size={12} /></span>
            </Button>
          )) : (
            <div className="grid h-64 place-items-center px-6 text-center text-xs leading-5 text-muted-foreground">{t("guardrails.emptyFolder")}</div>
          )}
        </div>
      </ScrollArea>
      ...
      <div className="border-t p-3">
        ...
        >
          <Keyboard aria-hidden size={14} />{t("guardrails.manualPath")}
        </Button>
        {manualOpen && (
          <div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
            <Input
              aria-label={t("guardrails.manualPathLabel")}
              className="min-h-11 font-mono text-xs"
              value={manualPath}
              onChange={(event) => setManualPath(event.target.value)}
            />
            <Button type="button" variant="outline" className="min-h-11" disabled={loading || !manualPath.trim()} onClick={() => void openDirectory(manualPath)}>{t("guardrails.openPath")}</Button>
          </div>
        )}
      </div>
    </div>
