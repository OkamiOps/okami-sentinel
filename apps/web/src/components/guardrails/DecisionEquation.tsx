  ...
  return (
    <section className="bench-panel min-w-0" aria-labelledby="decision-equation-title">
      <div className="border-b px-4 py-2.5">
        <div className="bench-label text-primary">{t("guardrails.equationTitle")}</div>
        <h2 id="decision-equation-title" className="sr-only">{t("guardrails.equationTitle")}</h2>
      </div>
        ...
        {nodes.map((node, index) => (
          <div key={node.id} className="contents">
            <div className="min-w-0 flex-1 border px-3 py-2.5">
              ...
              <div className={`mt-1 break-words text-xs font-medium ${node.id === "verdict" ? "lg:whitespace-nowrap" : ""}`}>{node.value || t("guardrails.undetermined")}</div>
            </div>
