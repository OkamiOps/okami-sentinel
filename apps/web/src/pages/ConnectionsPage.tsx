...
export function ConnectionsPage() {
  ...
  return <div>
    ...
    {connections === null ? <Loading label={t("connections.loading")} /> : connections.length === 0 ? <section className="bench-panel bench-corners"><EmptyState title={t("connections.empty")} description={t("connections.emptyDescription")} /></section> : <section className="bench-panel bench-corners min-w-0 overflow-hidden"><div className="grid min-w-0 lg:grid-cols-[minmax(18rem,.72fr)_minmax(0,1.28fr)]"><ConnectionList connections={connections} selectedId={selected?.id ?? null} onSelect={setSelectedId} /><ConnectionInspector connection={selected} onConnectionChange={(next) => setConnections((current) => (current ?? []).map((item) => item.id === next.id ? next : item))} onEdit={() => setEditor({ open: true, connection: selected })} onDelete={() => void remove()} deleting={deleting} /></div></section>}
