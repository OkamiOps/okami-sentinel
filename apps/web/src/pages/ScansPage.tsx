...
export function ScansPage() {
  ...
  const totalCostIsUpperBound = visible.some((scan) =>
    scanEstimatedUsd(scan) !== null && scan.cost?.estimateKind === "upper-bound"
  );
  ...
  return <div>
    ...
    <div className="bench-panel mb-4 grid sm:grid-cols-[minmax(0,1fr)_auto]">
      <label className="group flex h-11 items-center gap-3 border-b bg-background px-3 shadow-[inset_2px_0_0_transparent] transition-[border-color,background-color,box-shadow] focus-within:bg-primary/[.035] focus-within:shadow-[inset_2px_0_0_var(--primary)] sm:border-b-0 sm:border-r"><HugeiconsIcon icon={Search01Icon} size={14} className="text-muted-foreground transition-colors group-focus-within:text-primary" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("scans.filter")} className="h-full border-0 bg-transparent px-0 text-xs shadow-none hover:bg-transparent focus-visible:bg-transparent focus-visible:shadow-none" /></label>
    ...
    <div className="mb-4 grid grid-cols-2 border border-border lg:grid-cols-4"><LedgerReadout label="VISIBLE" value={visible.length} /><LedgerReadout label="EVIDENCE" value={evidence} /><LedgerReadout label="COST / USD" value={formatUsd(totalCost, totalCostIsUpperBound)} /><LedgerReadout label="ARCHIVED NOISE" value={scans.filter((s) => s.status === "cancelled" || s.status === "failed").length} /></div>
