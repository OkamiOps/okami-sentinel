...
import { formatDate, formatScanProgress, formatProgressLabel, formatProgressMetric, formatScanUsd, formatTokens, formatUsd, shortId } from "../format";
...
export function Telemetry({ scan, logs, logRef }: { scan: ScanRun; logs: string[]; logRef: React.RefObject<HTMLPreElement | null> }) {
  const { t } = useI18n();
  const activity = scan.status === "running" ? scan.progress?.activityState : undefined;
