...
export function ActivityPage() {
  ...
  return <div>
    ...
    <Panel className="mt-4" label="EVENT TRACE" title="Últimas transições">
      {history.length ? <div>{history.map((scan, i) => <div key={scan.id} className="grid grid-cols-[1.2rem_5rem_minmax(0,1fr)_auto] gap-3 border-b px-4 py-3 last:border-b-0 sm:grid-cols-[1.2rem_8rem_minmax(0,1fr)_8rem_auto]"><div className="relative flex justify-center"><span className="mt-1.5 size-1.5 rounded-full bg-border" />{i < history.length - 1 && <span className="absolute bottom-[-13px] top-4 w-px bg-border" />}</div><span className="font-mono text-[8px] text-muted-foreground">{formatDate(scan.completedAt ?? scan.startedAt)}</span><span className="min-w-0"><span className="block truncate text-xs font-medium">{scan.displayName}</span><span className="block truncate font-mono text-[8px] text-muted-foreground">{scan.model}/{scan.effort}</span></span><span className="hidden font-mono text-[9px] text-primary sm:block">{formatUsd(scan.cost?.estimatedUsd)}</span><Link to={`/scans/${scan.id}`} className="flex items-center gap-2"><StatusBadge status={scan.status} /><HugeiconsIcon icon={ArrowRight01Icon} size={11} className="text-muted-foreground" /></Link></div>)}</div> : <EmptyState title="Sem eventos" />}
    </Panel>
  </div>;
}
