...
export function ActivityPage() {
  ...
  return <div>
    ...
    <Panel label="LIVE BUS" title={t("activity.processes", { count: active.length })} aside={<span className="font-mono text-[9px] text-chart-2">POLL / 3.5S</span>}>
      {active.length ? <div className="divide-y">{active.map((scan, i) => <div key={scan.id} className="grid gap-4 px-4 py-4 lg:grid-cols-[3rem_minmax(0,1.3fr)_minmax(12rem,.7fr)_auto] lg:items-center"><div className="font-mono text-[9px] text-primary">PROC<br />{String(i + 1).padStart(2, "0")}</div><div className="min-w-0"><div className="flex items-center gap-2"><span className="truncate text-sm font-semibold">{scan.displayName}</span><StatusBadge status={scan.status} /></div><div className="mt-1 truncate font-mono text-[9px] text-muted-foreground">{scan.engine} · {scan.model}/{scan.effort} · {scan.repositoryPath}</div></div><div><div className="mb-2 flex items-center justify-between font-mono text-[9px]"><span className="text-muted-foreground">{scan.progress?.phaseLabel ?? "preflight"}</span><span className="text-primary">{scan.progress?.percent ?? 2}%</span></div><ProgressTrack value={scan.progress?.percent ?? 2} label={scan.progress?.detail ?? undefined} /></div><div className="flex items-center justify-end gap-2"><LiveDuration startedAt={scan.startedAt} status={scan.status} /><span className="font-mono text-[10px] text-primary">{formatUsd(scanEstimatedUsd(scan))}</span>{scan.status === "running" && <Button variant="destructive" size="icon-sm" onClick={() => void cancel(scan.id)} aria-label={t("activity.cancel")}><HugeiconsIcon icon={StopIcon} size={12} /></Button>}</div></div>)}</div> : <EmptyState title={t("activity.idle")} description={t("activity.idleDescription")} />}
    </Panel>
