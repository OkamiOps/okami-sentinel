  ...
  return <div>
    ...
    <div className="bench-panel bench-corners scanline overflow-hidden">
      <div className="grid border-b lg:grid-cols-[17rem_minmax(0,1fr)_19rem]">
        <div className="border-b lg:border-b-0 lg:border-r">
          <div className="flex h-11 items-center justify-between border-b px-3"><span className="bench-label">{t("dashboard.runChannels")}</span><span className="font-mono text-[9px] uppercase text-muted-foreground">{(data.recentTotal ?? data.totalScans) > channels.length ? t("dashboard.recentWindow", { shown: channels.length, total: data.recentTotal ?? data.totalScans }) : `${channels.length} ${t("dashboard.indexed")}`}</span></div>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(25rem,.75fr)]">
      <Panel label="TRACE 01" title={t("dashboard.costEvidence")} aside={<div className="flex gap-4 font-mono text-[8px]"><span className="text-primary">■ {t("dashboard.cost")}</span><span className="text-chart-2">— {t("common.findings")}</span></div>}>
        {(data.costTrendTotal ?? chart.length) > chart.length && <p className="px-4 pt-3 text-xs text-muted-foreground">{t("dashboard.trendWindow", { shown: chart.length, total: data.costTrendTotal! })}</p>}
...
function DashboardFilter({ value, onValueChange, label, allLabel, items }: { value: string; onValueChange: (value: string) => void; label: string; allLabel: string; items: Array<{ value: string; label: string }> }) {
  ...
  return <DropdownMenu>
    ...
    <DropdownMenuContent align="start" sideOffset={1} className="w-[var(--radix-dropdown-menu-trigger-width)] min-w-56 rounded-none border-border bg-popover p-1.5 shadow-[0_18px_60px_rgba(0,0,0,.35)] dark:shadow-[0_18px_60px_rgba(0,0,0,.65)]">
