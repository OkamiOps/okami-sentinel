...
export function NewScanPage() {
  ...
  return <div>
    <PageHeader code="03 / OPERATE" title={t("newScan.title")} description={t("newScan.description")} actions={<Button asChild variant="ghost" size="sm"><Link to="/scans"><HugeiconsIcon icon={ArrowLeft01Icon} size={12} />{t("newScan.back")}</Link></Button>} />
    ...
    {started && <AlertBanner tone="success">{t("newScan.accepted")} <Link to={`/scans/${started}`} className="underline underline-offset-4">{t("newScan.openActive")}</Link>.</AlertBanner>}
    <form onSubmit={(e) => void submit(e)}>
      <div className="grid gap-0 xl:grid-cols-[minmax(0,1.15fr)_minmax(20rem,.85fr)_19rem]">
        <Panel className="xl:border-r-0" label="STAGE 01 / TARGET" title={t("newScan.target")}>
          ...
          <div className="border-t p-4"><label className="bench-label" htmlFor="paths">SCOPE PATHS / OPTIONAL</label><Input id="paths" value={paths} onChange={(e) => setPaths(e.target.value)} className="mt-2 font-mono text-xs" placeholder="src, packages/api" /><p className="mt-2 text-[10px] text-muted-foreground">{t("newScan.scopeHelp")}</p></div>
        </Panel>

        <Panel className="xl:border-r-0" label="STAGE 02 / STRATEGY" title={t("newScan.strategy")}>
        ...
        <Panel className="bench-corners" label="STAGE 03 / AUTHORIZE" title={t("newScan.authorize")}>
          ...
          <div className="border-b p-4"><label className="bench-label" htmlFor="cost">MAX COST / USD</label><Input id="cost" type="number" min="100" step="1" value={maxCostUsd} onChange={(e) => setMaxCostUsd(e.target.value)} disabled={unlimited} className="mt-2 font-mono text-lg" /><label htmlFor="unlimited-cost" className="mt-3 flex cursor-pointer items-center gap-3 text-xs text-muted-foreground"><Checkbox id="unlimited-cost" checked={unlimited} onCheckedChange={(checked) => setUnlimited(checked === true)} />{t("newScan.unlimited")}</label></div>
          <div className="border-b p-4"><div className="bench-label">{t("newScan.requiredConfirmation")}</div><label htmlFor="authorize-scan" className={cx("mt-3 flex cursor-pointer items-start gap-3 border p-3 transition-colors", authorized ? "border-primary bg-primary/[.06]" : "border-primary/50 bg-background hover:border-primary hover:bg-accent/40")}><Checkbox id="authorize-scan" className="mt-0.5" checked={authorized} onCheckedChange={(checked) => setAuthorized(checked === true)} /><span><span className="block text-sm font-semibold">{t("newScan.authorizeExecution")}</span><span className="mt-1 block text-[11px] leading-relaxed text-muted-foreground">{model}/{effort}/{mode} · {t("newScan.estimatedCost")}.</span></span></label></div>
          <div className="p-4"><Button type="submit" size="lg" disabled={busy || !authorized} className="w-full justify-between">{busy ? t("newScan.starting") : t("newScan.submit")}<HugeiconsIcon icon={ArrowRight01Icon} size={13} /></Button><div className="mt-3 flex items-center justify-between font-mono text-[8px] text-muted-foreground"><span>ENGINE {health?.ok ? "READY" : "CHECK"}</span><span>{health?.activeScanIds.length ?? 0}/{health?.maxConcurrentScans ?? "—"} ACTIVE</span></div></div>
        </Panel>
      </div>
    </form>
  </div>;
