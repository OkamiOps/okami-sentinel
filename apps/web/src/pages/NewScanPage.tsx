    ...
    <form onSubmit={(e) => void submit(e)}>
      <div className="grid gap-0 xl:grid-cols-[minmax(0,1.15fr)_minmax(20rem,.85fr)_19rem]">
        ...
        <Panel className="xl:border-r-0" label="STAGE 02 / STRATEGY" title="Analysis profile">
          <div className="border-b p-4"><div className="bench-label mb-3">MODEL CHANNEL</div><div className="grid grid-cols-2">{models.map((id) => <button key={id} type="button" onClick={() => setModel(id)} className={cx("border px-3 py-4 text-left first:border-r-0", model === id ? "border-primary bg-primary/8" : "border-border hover:bg-accent")}><span className={cx("block font-mono text-[10px]", model === id && "text-primary")}>{id}</span><span className="mt-1 block text-[9px] text-muted-foreground">{id.includes("sol") ? "frontier / deep" : "balanced / daily"}</span></button>)}</div></div>
          <div className="border-b p-4"><div className="bench-label mb-3">REASONING EFFORT</div><div className="grid grid-cols-5">{efforts.map((id) => <button key={id} type="button" onClick={() => setEffort(id)} className={cx("h-14 border border-r-0 px-1 font-mono text-[8px] uppercase last:border-r", effort === id ? "bg-primary text-primary-foreground" : "hover:bg-accent")}>{id}</button>)}</div></div>
          ...
          <div className="p-4"><div className="bench-label">SELECTED PROFILE</div><div className="mt-3 border-l border-primary pl-3 font-mono text-[10px] leading-6"><div>{model}</div><div>{effort} / {mode}</div><div className="text-muted-foreground">paths: {paths ? paths.split(",").length : "all"}</div></div></div>
        </Panel>
