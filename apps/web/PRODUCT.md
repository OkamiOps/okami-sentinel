...
- A remote gate runs either from a Sentinel-managed immutable snapshot or from a repository-owned GitHub Actions caller pinned to one full release SHA. Remote policy is read-only in Sentinel; proposals are copied or downloaded and published through the repository's normal pull-request review. Only the workflow publishes GitHub Checks.
