...
- Portable keeps a server-owned dossier and emits findings-only report pages for at most 16 confirmed candidates each. Those private internal pages are validated and consolidated into one final report; rejected candidates and their coverage are server-derived. Under the maximum envelope, up to 32 pages and 512 normalized findings are supported. If a page, its coverage, its anchors, or final normalization fails, no partial final report is published.
- Portable Deep independently enumerates and partitions the immutable snapshot's auditable source and security-configuration files. Discovery completes only after every assigned file has been read completely; it never depends on a Standard run and never publishes partial Deep coverage. All Deep effort tiers share one 90-minute scan deadline while turn and tool envelopes scale from 48/384 through 128/1,024.
