...
- Un gate distant s’exécute depuis un snapshot immuable géré par Sentinel ou un caller GitHub Actions appartenant au dépôt et épinglé sur un SHA complet de release. Sentinel peut installer ou mettre à jour ce caller via la GitHub App autorisée et conserve dans le workflow les déclencheurs push, pull request et post-merge choisis. La policy distante reste en lecture seule ; seul le workflow publie les GitHub Checks.
...
- Portable conserve un dossier géré par le serveur et n’émet que des pages de rapport findings-only de 16 candidats confirmés au maximum chacune. Ces pages internes privées sont validées puis consolidées dans un seul rapport final ; les candidats rejetés et leur couverture sont dérivés par le serveur. Avec l’enveloppe maximale, jusqu’à 32 pages et 512 findings normalisés sont pris en charge. Si une page, sa couverture, ses ancres ou la normalisation finale échouent, aucun rapport final partiel n’est publié.
- Portable Deep énumère et partitionne de façon autonome les fichiers auditables de code source et de configuration de sécurité du snapshot immuable. La découverte ne se termine qu’après lecture complète de chaque fichier attribué ; elle ne dépend jamais d’un run Standard et ne publie aucune couverture Deep partielle. Tous les groupes d’effort Deep partagent un deadline global de 90 minutes, tandis que les enveloppes de tours et d’outils évoluent de 48/384 à 128/1 024.
