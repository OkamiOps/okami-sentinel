...
Portable `deep` ermittelt die prüfbaren Quell- und Sicherheitskonfigurationsdateien im unveränderlichen Snapshot und stellt vollständige Dateien in serverseitigen Discovery-Partitionen bereit. Jede Partition muss abgeschlossen werden. Dies belegt bereitgestellte Quellabdeckung, nicht das Verständnis jeder Zeile durch das Modell. Ein vorheriges `standard`-Ergebnis wird nicht vorausgesetzt. Portable `standard` nutzt begrenzte, gezielte Exploration und muss geprüfte und ungeprüfte Bereiche ehrlich ausweisen; dieselbe Abdeckung wie Deep wird nicht versprochen.
...
Dataflow und Validation verarbeiten übernommene Kandidaten in servergeführten Seiten mit höchstens 32 Kandidaten. Der Report-Planer erzeugt die für bestätigte Kandidaten erforderlichen privaten Seiten; der kanonische Normalisierer akzeptiert bis zu 512 Findings und veröffentlicht daraus genau ein normalisiertes `findings.json`. Seitenzuordnung und Repository-Anker werden vor I/O geprüft, sodass ein dichter Deep-Lauf weder Kandidaten still abschneidet noch eine partielle Seite veröffentlicht.
...
|---|---:|---:|
| `minimal`, `low` | 24 Turns / 96 Tool-Aufrufe | 48 Turns / 384 Tool-Aufrufe |
| `medium`, `high`, unbekannt oder nicht gesetzt | 32 Turns / 128 Tool-Aufrufe | 64 Turns / 512 Tool-Aufrufe |
| `xhigh` | 48 Turns / 192 Tool-Aufrufe | 96 Turns / 768 Tool-Aufrufe |
| `max`, `ultra` | 64 Turns / 256 Tool-Aufrufe | 128 Turns / 1.024 Tool-Aufrufe |

Keiner der Modi endet allein wegen verstrichener Zeit. Abbruch bleibt möglich; eine optionale Kostenobergrenze gilt für den gesamten Scan. Turn- und Tool-Werte begrenzen eine Basissitzung; Discovery-, Bewertungs- und Berichtsseiten haben eigene begrenzte Regeln, ohne das Kostenbudget zurückzusetzen. Portable-Basissitzungen sind auf 64 MiB Eingabe und 1 MiB Ausgabe begrenzt; vollständige Deep-Coverage-Partitionen dürfen einen eigenen 16-MiB-Ausgabe-Guard verwenden, während der kanonische öffentliche Bericht auf 4 MiB begrenzt bleibt. Eine optionale USD-Obergrenze verwendet ein eingefrorenes passendes Preisangebot; ohne ein solches Angebot blockiert Sentinel den Start. Sobald gemeldete Nutzung die Obergrenze erreicht, endet die Sitzung vor der nächsten Provider-Anfrage, auch wenn eine bereits laufende Anfrage die Schätzung noch darüber heben kann. Lässt sich die Usage während des Laufs nicht mehr schätzen, beendet Sentinel die Sitzung, statt Kosten oder eine durchgesetzte Obergrenze zu erfinden.
