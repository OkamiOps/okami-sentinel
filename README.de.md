...
Portable `deep` ist eigenständig vollständig und verwendet oder erwartet niemals ein vorheriges `standard`-Ergebnis. Sentinel ermittelt im unveränderlichen Snapshot die prüfbare Menge aus Quellcode und Sicherheitskonfiguration, partitioniert sie serverseitig und verlangt für jede zugewiesene Datei einen vollständigen erfolgreichen `workspace.read`, bevor Discovery enden darf. Eine fehlende Datei oder Partition führt fail-closed zum Abbruch, statt einen partiellen Deep-Bericht zu veröffentlichen. `standard` bleibt der kostengünstigere begrenzte Explorationsmodus und verspricht daher nicht dieselbe vollständige Abdeckung.
