...
Neue Portable-Discovery-Artefakte benötigen ein explizites `candidates`-Array und begrenzten Kontext: Hypothese, Angreifer, Voraussetzungen, erwartete Auswirkungen und vermuteter Kontrollfehler. Die Validierung darf Aufrufer und Schutzmaßnahmen in anderen Dateien prüfen. Unbelegte Hypothesen bleiben als `insufficient-evidence` außerhalb bestätigter Findings; sie werden nicht automatisch zu medium/low-Schwachstellen. Neue high/critical-Findings benötigen eine Schweregradbegründung. Diese Evidenzprüfungen garantieren keine Modellgenauigkeit.

Die Berichtsphase teilt bestätigte Kandidaten intern in Seiten mit höchstens 4 Kandidaten auf. Eine Modellseite darf nur ihre Findings liefern; Coverage und abgelehnte Kandidaten bleiben servergeführt. Vor jedem `results.write` prüft Sentinel JSON, Stufenvertrag, Dossier-Semantik, Seitenzuordnung, Coverage sowie Pfade und Zeilenbereiche der Snapshot-Anker. Abgelehnte Artefakte erhalten konkrete Korrekturhinweise; wiederholt identische ungültige Schreibversuche unterliegen weiterhin der Stagnationsprüfung. Nur der serverseitig zusammengesetzte Abschlussbericht wird als `sentinel-findings.json` geschrieben; Seitenartefakte werden nicht als Sentinel-Ergebnis veröffentlicht.

Dataflow verarbeitet Kandidaten in servergeführten Seiten mit höchstens 32 Kandidaten; Validation verwendet Seiten mit höchstens acht. Der Report-Planer erzeugt die für bestätigte Kandidaten erforderlichen privaten Seiten; der kanonische Normalisierer akzeptiert bis zu 512 Findings und veröffentlicht daraus genau ein normalisiertes `findings.json`. Seitenzuordnung und Repository-Anker werden vor I/O geprüft, sodass ein dichter Deep-Lauf weder Kandidaten still abschneidet noch eine partielle Seite veröffentlicht.
...
### Graphify, Abdeckung und Wiederherstellung

Neue vollständige Standard- und Deep-Scans erfassen dieselben prüfbaren Quell- und Konfigurationsdateien und stellen zugewiesene Dateien vollständig bereit. Graphify gruppiert zusammenhängende Dateien; Dateien ohne Graphknoten bleiben enthalten. Standard prüft Hauptpfade mit standardmäßig `low`, Deep untersucht alternative Pfade und Gegenbelege mit `high`. Explizite unterstützte Optionen bleiben erhalten; ohne Effort-Steuerung gilt der Provider-Standard. Bereitgestellter Code beweist weder Modellaufmerksamkeit noch Erkennungsqualität. Jeder vollständige Scan ist unabhängig; die Wiederherstellung verwendet nur validierte Checkpoints dieses Laufs.

Sentinel verwaltet Graphify: pnpm stellt die isolierte Laufzeit bereit, Docker enthält sie im Image. Quellprojektionen für Bewertung und Bericht passen sich dem verfügbaren Kontext an, mit Reserven für Anweisungen, Ausgabe und Fortsetzung. Es gibt keine feste Anzahl von Ausschnitten oder maximale Funktionslänge; ausstehende Bereiche bleiben gezielt lesbar. Die geschätzte Kontextgrenze ist das Minimum aus 300K Tokens und Modellfenster. Graphbeziehungen dienen der Navigation, nicht als Schwachstellenbeweis.

Standard und Deep haben keine kumulative Grenze für Tool-Aufrufe oder Modell-Turns. Wiederholte Inspektionen erhalten Hinweise gegen Schleifen; produktive Untersuchung darf fortfahren. Evidenzprüfung, Byte-/Kontextkontrollen, Abbruch und optionale scanweite Kostengrenzen bleiben aktiv. Behebbare Fehler verwenden kleinere Einheiten mit frischem Verlauf und validierten Checkpoints innerhalb dauerhafter Wiederholungsbudgets. Terminale Fehler und explizite Abbrüche werden nicht blind wiederholt. Die Live-Vorschau zeigt vorläufige Kandidaten, keine zusätzlich bestätigten Findings.

Siehe [Architektur und Messergebnisse](docs/architecture/portable-graph-recovery.md). Der neue adaptive Deep benötigte rund 5h21 für 36 gemeldete Findings, gegenüber 7h07 und 31 auf demselben Snapshot mit der vorherigen Graphify-Integration. Das ist eine beobachtete Verbesserung, kein isoliertes Experiment oder Genauigkeitsnachweis. Standard wurde mit der neuesten adaptiven Projektion noch nicht gemessen. Die Benchmark-Ziele von 90 Minuten für Standard und fünf Stunden für Deep sind keine Laufzeit-Timeouts.
...
Ändernde Aufrufe der lokalen API benötigen das Prozesstoken von `GET /api/security-session` im Header `X-CSRF-Token`, auch bei CLI-Clients. Browser-Ursprünge sind auf die lokale Anwendung und das unterstützte Vite-Frontend beschränkt; die Oberfläche sendet das Token automatisch.
...
In den Branch-Protection-Regeln muss der exakte Check-Name **`CSB Security Change Gate`** stehen. `CSB_GITHUB_ACTIONS_WORKFLOW_SHA` muss denselben unveränderlichen SHA enthalten, bevor Sentinel den Actions-Executor als bereit einstuft oder ausführt.
...
| `CSB_HOST` | `127.0.0.1` | Nur Loopback-Adressen; öffentliche/LAN-Adressen werden abgelehnt, da die lokale API keine globale Authentifizierung hat. Keine Freigabe über Proxy oder Tunnel ohne authentifizierte Zugriffsschicht. |
| `CSB_PORT` | `8787` | API-Port |
| `CSB_GITHUB_ACTIONS_WORKFLOW_SHA` | nicht verfügbar | Unveränderlicher 40-stelliger Sentinel-Release-SHA für Remote-Caller |
...
Die Browsertests prüfen Desktop/Mobilgeräte und alle fünf Oberflächensprachen mit einer simulierten API. Sie starten keine externen Scans und verwenden keine echten Anbieterzugangsdaten.
